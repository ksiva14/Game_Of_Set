...
class Cards {
    ...
    */
    clicked_behavior(card_deck, playerCheck) {
        let player;
        if (playerCheck == 1) {
            player = player1;
        } else {
        ...
        if ((!isNaN(card_deck.selected_index)) && (card_deck.array_index.length < 3)) {
          ...
          this.add_to_selected(card_deck);
        }
        //Check if 3 cards are selected

        if ((card_deck.array_index.length == 3) && (card_deck.is_set(card_deck.cards_selected))) {
          ...
          /*
          for (let i = 0; i < 3; i++) {
            document.getElementById(String(card_deck.array_index[i] + 1)).remove();
          }
          */
        ...
        } else if ((card_deck.array_index.length == 3) && (!card_deck.is_set(card_deck.cards_selected))) {
     ...
     print_cards(card_deck) {
      ...
      for (let i = 0; i < num_cards; i++) {
        ...
        for (let j = 0; j < 6; j++) {
            ...
            c_node.addEventListener("click", () => curr_card.clicked(card_deck, c_node));
            index++;
        }
      }
      //Display last row
...
class Deck {
  constructor() {
    ...
    this.cards_selected = [];
    // array of cards displayed to player
    this.cards_displayed = [];
  ...
  /* Function to find if there are any sets remaining in the deck
    ...
    replace_card () {
      ...
      this.array_index.forEach( i => {
        ...
        /* get a new card */
        if (!this.deck_empty(this.deck_of_cards) && (this.current_deck.length <= 12)) {
...
class GUI {
  ...
  addButtonListeners() {
    ...
    this.changePlayerButton.addEventListener("click", () => {
      let selected_player = game.current_player.playerNum;
      console.log("selected " + game.current_player.playerNum);
      game.current_player.select();
...
class Player {
  ...
  update_score() {
    ...
    document.getElementsByClassName("player-score")[
      this.playerNum
