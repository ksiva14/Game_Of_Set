...
class Deck {
  ...
  replace_card() {
    this.array_index.forEach((i) => {
      let index = parseInt(this.array_index[i]);
      /* get a new card */
      if (
...
class GUI {
  ...
  delete_sprite(card_deck, index) {
    for (let i = 0; i < 3; i++) {
      //console.log(card_deck.cards_displayed.length);
      //card_deck.cards_displayed.splice(index, 1);
    }
  }
