...
class Cards {
    ...
    clicked() {
      ...
      Game.card_deck.select_cards(Game.card_deck, Game.current_player);
    }
    ...
    */
    clicked_behavior(card_deck, player) {
        ...
        if ((card_deck.index_array.length == 3) && (card_deck.is_set(card_deck.selected_cards))) {
          ...
          let counter = 0;
          for (c of card_deck.selected_cards) {
            c.selected = false;
        ...
        for (let j = 0; j < this.shape.length; j++) {
          for (let k = 0; k < this.texture.length; k++) {
            for (let l = 0; l < this.color.length; l++) {
              ...
              this.deck_of_cards.push(new Cards(this.color[l], this.texture[k], this.shape[j], this.numbers[i], image_path));
            }
          }
        }
      }
...
class Game {
    ...
    setup_deck() {
      ...
      window.alert("TEST1" + card_deck.deck_of_cards);
      card_deck.create_card_deck();
      window.alert("TEST2" + card_deck.deck_of_cards[0].color);
      this.deal_cards();
    }
